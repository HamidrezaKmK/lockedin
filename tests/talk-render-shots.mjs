...
async function main(){
  ...
  try{
    ...
    await p.addInitScript(()=>{
      // Deterministic and offline: captureNote sees the same canvas contract as html2canvas,
      // while this suite can assert the PNG's dimensions after applying a crop.
      window.html2canvas=async el=>{
        const c=document.createElement("canvas");
        c.width=Math.max(1,Math.round(el.clientWidth*2));
        c.height=Math.max(1,Math.round(el.clientHeight*2));
        const x=c.getContext("2d");x.fillStyle="#182235";x.fillRect(0,0,c.width,c.height);
        return c;
      };
    });
    p.on("pageerror",e=>errs.push("pageerror: "+e.message));
    p.on("console",m=>{if(m.type()==="error")errs.push("console: "+m.text().slice(0,160));});
    let fullTalkGets=0,statusGets=0;
    p.on("request",request=>{
      const pathname=new URL(request.url()).pathname;
      if(request.method()==="GET"&&pathname===`/api/bubbles/${slug}/talks/${talkId}`)fullTalkGets++;
      if(request.method()==="GET"&&pathname===`/api/bubbles/${slug}/talks/${talkId}/status`)statusGets++;
    });
    ...
    const goSlide=async n=>{
      // A talk intentionally keeps a tiny status poll alive; DOM readiness plus the actual
      // slide selector is the meaningful loaded condition, while network-idle is not.
      await p.goto(`${base}/#bubble/${slug}/talk/${encodeURIComponent(talkId)}/slide/${n}`,{waitUntil:"domcontentloaded"});
      await p.waitForSelector(".tk-slide",{timeout:15000}); await delay(700);
    };
    await goSlide(1);
    const fullAfterLoad=fullTalkGets,statusAfterLoad=statusGets;
    await delay(11_000);
    assert.equal(fullTalkGets,fullAfterLoad,
      "five-second talk polling must not download the full deck again");
    assert.ok(statusGets-statusAfterLoad>=2,
      `expected lightweight status polls, saw ${statusGets-statusAfterLoad}`);
    console.log("PROBE sustained polling: status only, no repeated full-deck downloads");
    ...
    // Collapse is a reading preference, scoped to this user/workspace/bubble. It survives both
    // an ordinary reload and a complete logout/login cycle without changing the mark itself.
    const savedMark=p.locator(".tk-note").first();
    await savedMark.getByRole("button",{name:"Collapse mark"}).click();
    await p.reload({waitUntil:"domcontentloaded"});await p.waitForSelector(".tk-note.collapsed",{timeout:10000});
    await p.evaluate(async({user})=>{
      await fetch("/api/logout",{method:"POST",headers:{"Content-Type":"application/json"}});
      await fetch("/api/login",{method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({username:user,password:"temporary-password"})});
    },{user});
    await p.reload({waitUntil:"domcontentloaded"});await p.waitForSelector(".tk-note.collapsed",{timeout:10000});
    console.log("PROBE collapsed mark: persisted across reload and logout/login");
    await savedMark.getByRole("button",{name:"Expand mark"}).click();

    // Resolve must feel immediate even if disk/job archival is slow. Delay the response by a
    // full second and require the card to disappear while that request is still in flight.
    await p.route(/\/notes\/[^/]+\/status$/,async route=>{await delay(1000);await route.continue();});
    const resolveResponse=p.waitForResponse(r=>r.request().method()==="PATCH"&&/\/notes\/[^/]+\/status$/.test(new URL(r.url()).pathname));
    const resolveStarted=Date.now();
    await savedMark.getByRole("button",{name:"resolve",exact:true}).click();
    await savedMark.waitFor({state:"hidden",timeout:300});
    assert.ok(Date.now()-resolveStarted<300,"resolve did not update the interface optimistically");
    assert.ok((await resolveResponse).ok(),"the delayed resolve request failed");
    await p.unroute(/\/notes\/[^/]+\/status$/);
    console.log("PROBE resolve: card hid immediately and stayed resolved after the delayed response");

    // Draw two strokes, undo the second with the platform shortcut, then shrink the outer
    // screenshot box. The API receives one path and a crop, and the uploaded PNG is smaller
    // than the slide-sized capture generated by the deterministic stub above.
    const slideBox=await p.locator(".tk-slide").boundingBox();
    assert.ok(slideBox,"the drawing slide has no layout box");
    await p.locator("[data-ink]").click();
    const stroke=async(points)=>{
      await p.mouse.move(points[0][0],points[0][1]);await p.mouse.down();
      for(const [x,y] of points.slice(1))await p.mouse.move(x,y,{steps:4});
      await p.mouse.up();
    };
    await stroke([[slideBox.x+120,slideBox.y+150],[slideBox.x+260,slideBox.y+210]]);
    await stroke([[slideBox.x+310,slideBox.y+260],[slideBox.x+430,slideBox.y+320]]);
    await p.keyboard.press("Control+z");
    await p.locator(".tk-inkbar [data-done]").click();
    await p.waitForSelector(".tk-cropbox",{timeout:2000});
    await shot("drawing-crop-box",".tk-slide");
    const cropBox=await p.locator(".tk-cropbox").boundingBox();
    assert.ok(cropBox,"crop box has no layout box");
    await p.mouse.move(cropBox.x+cropBox.width-7,cropBox.y+cropBox.height-7);
    await p.mouse.down();await p.mouse.move(cropBox.x+cropBox.width*.62,cropBox.y+cropBox.height*.64,{steps:5});
    await p.mouse.up();
    await p.locator(".tk-inkbar [data-done]").click();
    await p.waitForSelector(".tk-pop",{timeout:2000});
    const noteResponse=p.waitForResponse(r=>r.request().method()==="POST"&&
      new URL(r.url()).pathname===`/api/bubbles/${slug}/talks/${talkId}/notes`);
    const shotResponse=p.waitForResponse(r=>r.request().method()==="PUT"&&/\/shot\.png$/.test(new URL(r.url()).pathname));
    await p.locator(".tk-pop [data-pin]").click();
    const inkNote=(await (await noteResponse).json()).note;
    assert.equal(inkNote.paths.length,1,"Ctrl+Z must remove the most recent stroke");
    assert.ok(inkNote.rect&&inkNote.rect.w<75&&inkNote.rect.h<75,
      `expected a smaller screenshot crop, got ${JSON.stringify(inkNote.rect)}`);
    const uploaded=await shotResponse;assert.ok(uploaded.ok(),"cropped drawing snapshot upload failed");
    const image=await ctx.request.get(`${base}/api/bubbles/${slug}/talks/${talkId}/notes/${inkNote.id}/shot.png`);
    const png=await image.body();
    const pngWidth=png.readUInt32BE(16),pngHeight=png.readUInt32BE(20);
    assert.ok(pngWidth<slideBox.width*1.5&&pngHeight<slideBox.height*1.5,
      `cropped PNG remained slide-sized (${pngWidth}x${pngHeight})`);
    console.log("PROBE drawing: Ctrl+Z kept one stroke and the adjusted crop produced a smaller PNG");
    ...
    // Carry that current session to the mobile context, not the pre-logout seed cookie.
