"""Deterministic regression tests for the save pipeline and asset ingestion.

No network / no LLM: where a model is needed (asset summarization) it is replaced with a
canned response so every case is exact and reproducible.

The AI chat was removed entirely — the model layer now exists only to summarize uploaded
PDFs, and reports are written by the user (editor or Scientist client). These tests pin:

* wikilink normalization on save (title/prefix -> real slug);
* display-math normalization on save + mtime stability + the figure-change poll signal;
* save conflict detection, TODOs, citations, figures, review comments;
* asset metadata + resummarization (the model's one remaining job);
* that the removed chat surface stays removed (no routes, no frontend pane).
...
# --------------------------------------------------------------------------- #
# The chat stays removed
# --------------------------------------------------------------------------- #
class ChatSurfaceStaysRemoved(unittest.TestCase):
    """The research chat was deleted end to end. These pins keep it from creeping back:
    the model layer's only consumer is asset summarization."""
    ...
    def test_chat_routes_are_gone(self):
        with tempfile.TemporaryDirectory() as directory, patch.dict(
            ...
            with TestClient(server.build_app(), base_url="https://testserver") as client:
                self.assertEqual(client.post(
                    "/api/signup", json={"username": "nochat", "password": "testpass"}).status_code, 200)
                for method, route in [("POST", "/api/bubbles/x/chat"),
                                      ("GET", "/api/bubbles/x/chats"),
                                      ("POST", "/api/bubbles/x/chats/title"),
                                      ("POST", "/api/slack/ask")]:
                    r = client.request(method, route, json={})
                    self.assertIn(r.status_code, (404, 405), route)

    def test_backend_chat_machinery_is_gone(self):
        from lockedin import reports
        for name in ("chat_stream", "generate_chat_title", "CHAT_SYSTEM"):
            self.assertFalse(hasattr(reports, name), name)
        for name in ("attach_pdf", "compact_chat"):
            self.assertFalse(hasattr(models, name), name)
        for name in ("list_chat_sessions", "save_chat_session"):
            self.assertFalse(hasattr(bubbles, name), name)
        # complete() must survive — it is the tagger's summarization entry point.
        self.assertTrue(callable(models.complete))

    def test_frontend_has_no_chat_pane(self):
        src = (Path(server.__file__).parent / "web" / "index.html").read_text(encoding="utf-8")
