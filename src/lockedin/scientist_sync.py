...
def _indexed_context(slug: str) -> dict[str, bytes]:
    ...
    # A large asset is invisible from the project folder: it is not on disk and an `ls` shows
    # nothing where it would be. One generated file says what is missing and how to move it — one
    # marker rather than a stub per asset, so there is a single thing to keep out of the push
    # path instead of one per file.
    marker = _not_synced_markdown(asset_root)
    if marker:
        out[NOT_SYNCED_PATH] = marker
...
NOT_SYNCED_NAME = "NOT-SYNCED.md"
NOT_SYNCED_PATH = f"reports/assets/{NOT_SYNCED_NAME}"
...
def _not_synced_markdown(asset_root: Path) -> bytes | None:
    """The marker file, or None when every asset in this bubble syncs normally."""
    if not asset_root.exists():
        return None
    big = sorted((p for p in asset_root.iterdir()
                  if p.is_file() and not p.name.endswith(".tmp") and _is_large(p)),
                 key=lambda p: p.name)
    if not big:
        return None
    rows = "\n".join(f"| `{p.name}` | {_size_label(p.stat().st_size)} |" for p in big)
    total = _size_label(sum(p.stat().st_size for p in big))
    body = f"""<!-- Generated by LockedIn. Do not edit: this file is rewritten on every sync. -->
# Files this folder does not sync

{len(big)} file(s) in this bubble, {total} in total, are larger than
{_size_label(LARGE_ASSET_BYTES)} and are therefore never carried by the sync in either
direction — re-reading them on every poll would cost more than the whole rest of the bubble.

They exist in the bubble. They are simply not on disk here unless someone fetched them, which
is why this file exists: without it nothing in this folder would show that they are there.
...
## Moving one
    ...
    lockedin-scientist assets                     # which side each file is on
    lockedin-scientist assets pull <filename>     # bring one down here
    lockedin-scientist assets push <filename>     # send a local one up
    lockedin-scientist assets rm <filename>       # delete it from the bubble

Deleting a large file from this folder does not remove it from the bubble; `assets rm` is what
does that. Adding one here does not upload it; `assets push` does.
"""
...
def writable_path(slug: str, rel: str) -> bool:
    ...
    # real file in the bubble.
    if rel == NOT_SYNCED_PATH:
        return False
    if parts[1] == "assets":
